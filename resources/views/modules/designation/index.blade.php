...
<div class="content">
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            <h3 class="block-title">Designation</h3>
            @permission('add-designation')
            <button type="button" class="btn btn-outline-primary push" data-toggle="modal" data-target="#add-modal">Add New Designation</button>
            @endpermission
        </div>
...
@include('designation::includes._modal_edit')
@include('designation::includes._modal_add')
@endsection
@section('styles')
...
<script>
    ...
    $(document).on('click', '[id=btn-edit]', function(){
        var id = $(this).data('id');
        var show_route = '{{ route('designation.show', ':id') }}';
        var update_route = '{{ route('designation.update', ':id') }}';
