...
<div class="content">
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            <h3 class="block-title">Branch</h3>
            {{-- @ability('Admin', 'add_branch') --}}
            <button type="button" class="btn btn-outline-primary push" data-toggle="modal" data-target="#add-modal">Add New Branch</button>
            {{-- @endability --}}
        </div>
...
@include('branch::includes._modal_edit')
@include('branch::includes._modal_add')
@endsection
@section('styles')
...
<script>
    ...
    $(document).on('click', '[id=btn-edit]', function(){
        var id = $(this).data('id');
        var show_route = '{{ route('branch.show', ':id') }}';
        var update_route = '{{ route('branch.update', ':id') }}';
