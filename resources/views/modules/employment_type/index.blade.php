...
@section('breadcrumbs')
{{ Breadcrumbs::render('employment-type.index') }}
@endsection
@section('content')
<div class="content">
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            <h3 class="block-title">Employment Type</h3>
            {{-- @ability('Admin', 'add_employment_type') --}}
            <button type="button" class="btn btn-outline-primary push" data-toggle="modal" data-target="#add-modal">Add New Employment Type</button>
            {{-- @endability --}}
        </div>
...
@include('employment_type::includes._modal_edit')
@include('employment_type::includes._modal_add')
@endsection
@section('styles')
...
<script>
    jQuery("#datatable").dataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route('employment-type.index') }}',
        columns : [
            {data: 'name'},
            {data: 'description'},
    ...
    $(document).on('click', '[id=btn-edit]', function(){
        var id = $(this).data('id');
        var show_route = '{{ route('employment-type.show', ':id') }}';
        var update_route = '{{ route('employment-type.update', ':id') }}';
